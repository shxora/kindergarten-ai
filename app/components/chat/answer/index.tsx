...
function OperationBtn({ innerContent, onClick, className }: { innerContent: React.ReactNode, onClick?: () => void, className?: string }) {
  return (
    <div
      className={cn(
        'relative box-border flex h-7 w-7 items-center justify-center rounded-lg bg-white p-0.5 cursor-pointer text-gray-500 hover:text-gray-800',
        className ?? '',
      )}
...
const Answer: FC<IAnswerProps> = ({
  ...
  const renderFeedbackRating = (rating: MessageRating | undefined) => {
    ...
    return (
      ...
      >
        {OperationBtn({
          innerContent: <div className={`${ratingIconClassname} rounded-lg h-6 w-6 flex items-center justify-center`}><RatingIcon isLike={isLike} /></div>,
          onClick: async () => {
  ...
  const renderItemOperation = () => {
    ...
    return (
      <div data-maiya-item-operation className={`${s.itemOperation} flex gap-1`}>
        <Tooltip
          selector={`user-feedback-${randomString(16)}`}
          content={isLike ? '取消赞同' : (t('common.operation.like') as string)}
        >
          {OperationBtn({
            innerContent: (
              <div className={`${isLike ? 'text-primary-600 bg-primary-100 hover:bg-primary-200' : ''} rounded-lg h-6 w-6 flex items-center justify-center`}>
                <RatingIcon isLike={true} />
              </div>
            ),
            onClick: () => onFeedback?.(id, { rating: isLike ? null : 'like' }),
          })}
        </Tooltip>
        <Tooltip
          selector={`user-feedback-${randomString(16)}`}
          content={isDislike ? '取消反对' : (t('common.operation.dislike') as string)}
        >
          {OperationBtn({
            innerContent: (
              <div className={`${isDislike ? 'text-red-600 bg-red-100 hover:bg-red-200' : ''} rounded-lg h-6 w-6 flex items-center justify-center`}>
                <RatingIcon isLike={false} />
              </div>
            ),
            onClick: () => onFeedback?.(id, { rating: isDislike ? null : 'dislike' }),
          })}
        </Tooltip>
      </div>
    )
        ...
        <div data-maiya-answer-wrap className={`${s.answerWrap} max-w-[calc(100%-3rem)]`}>
          <div className={`${s.answer} relative text-sm text-gray-900`}>
            ...
            <div data-maiya-actions className="absolute bottom-[-14px] right-[-14px] flex flex-row justify-end gap-1">
                ...
                !isResponding && content && (
                  <div className="maiya-copy-button">
                    <Tooltip selector={`copy-answer-${randomString(16)}`} content={copied ? '已复制' : '复制内容'}>
                      {OperationBtn({
                        innerContent: copied
                          ? <IconWrapper><RiCheckLine className="h-4 w-4 text-green-500" /></IconWrapper>
                          : <IconWrapper><RiFileCopyLine className="h-4 w-4" /></IconWrapper>,
                        onClick: handleCopy,
              ...
              {!isResponding && !feedbackDisabled && !item.feedbackDisabled && renderItemOperation()}
            </div>
          </div>
        </div>
      </div>
